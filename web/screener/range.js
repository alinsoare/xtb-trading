...
export function computeRange(d1Bars, price) {
  if (!d1Bars?.length || price == null) {
    return { high: null, low: null, rangePct: null, positionPct: null, headroomPct: null };
  }
  ...
  const span = high - low;
  if (span <= 0) {
    return { high, low, rangePct: null, positionPct: null, headroomPct: null };
  }

  // Headroom is measured over the current price, not the window's low — same convention as
  // the pivot-distance rule in score.js: (high − price) / price.
