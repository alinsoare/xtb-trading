...
**Delegation**: When the user invokes this command directly, run the workflow in the `opsx-syncer` subagent rather than in this thread. Hand it the change name, any narrowed delta-path selection, and any context from this conversation it needs, since it starts with an empty context window. Do not do the work yourself unless the subagent is unavailable, in which case fall back to the steps below. When it returns, relay its report. This does not apply when the archive workflow invokes sync inline - that runs in place, already inside its own subagent.
